...
- Builtins: the available operations (opcodes), their types, and tightening
  rules.
- Channel Lattices: partial order used by channelGeq.
...
- PSP-2/PSP-4 MAY define domain vocabularies and trust registries (e.g., action
  ...
  and are asserted in Programs (e.g., via ctxEq) or are enforced by acceptance
  policy (TAP).
...
- A channel lattice defines a partial order over channel profiles (e.g.,
  ...
  by the builtin `channelGeq(channel, floor)`. `channelLatticeId` is the
  content-addressed ID of the specific lattice used to interpret ">=". It is
  required whenever a Program uses `channelGeq`.

Builtins and `builtinsId`:

- The builtins registry defines each opcode's semantics, types, and attenuation
  tightening rules (e.g., withinTime, ttlOk, inPairSet, channelGeq, ctxEq,
  presenterIs).
...
#### 6.2.1 Channel Binding Profiles

The channel lattice enumerates recognized channel binding profiles and defines a
partial order over them. Each profile string corresponds to a concrete
sender-constraining mechanism from external standards. Common profiles include:

- **mtls:v1** - Mutual TLS client authentication as defined by TLS 1.3 (RFC
  8446).
- **tls-exporter:v1** - TLS 1.3 exporter-based channel binding (RFC 9266).
- **dpop:v1** - Demonstrating Proof of Possession (DPoP) sender-constraining
  (RFC 9449).
- **bearer:v1** - No sender-constraining (OAuth 2.0 Bearer tokens).

The partial order often follows the relative strength of sender-constraining,
for example: `mtls:v1` >= `tls-exporter:v1` >= `dpop:v1` >= `bearer:v1`. The
exact order and the set of recognized profiles are defined in the pinned channel
lattice artifact referenced by `channelLatticeId`. A Program using `channelGeq`
MUST pin a `channelLatticeId` that includes the relevant profiles.

#### 6.2.2 Snapshot Structure

To prevent comparator drift, `schemesSnapshotId` pins a manifest mapping scheme
names to the exact comparator artifacts used for normalization and subset
comparison. An example snapshot might look like:

```json
{
  "vault": "cid:comparator-vault@2025-09-01",
  "k8s": "cid:comparator-k8s@2025-08-15",
  "door": "cid:comparator-door@2025-09-01"
}
```

Each value is a content-addressed identifier of a comparator implementation.
Comparators MUST remain pure, deterministic, and bounded; unknown or mismatched
comparators across delegation hops MUST cause deny. The snapshot itself is
pinned by `schemesSnapshotId` in the Grant's `pins` field.
...
- `builtinsId`: content-addressed identifier of the Builtins set used by the
  Program.
- `channelLatticeId`: REQUIRED if the Program contains `channelGeq`; otherwise
  OPTIONAL.
...
drift (e.g., tightening changes to withinTime or reordering of channel
...
1. Payload contents
   ...
   - `pins`: Registry pins that fix semantics across CEPs and time:
     ...
     - `channelLatticeId`: REQUIRED if and only if the Program contains
       `channelGeq` literals; OPTIONAL otherwise. When present, it pins the
...
- ctx: `Map<Str, Term>` - runtime context; MUST include at least the k/v
  required by the Program's ctxEq literals.
