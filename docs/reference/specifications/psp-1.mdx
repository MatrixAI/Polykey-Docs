...
- **`builtinsId`** - a content-addressed snapshot of the builtin operators in
  ...
  resource bounds).
- **`schemesSnapshotId`** - a content-addressed manifest that maps each scheme
  name referenced by this Grant's declarations to the exact comparator snapshot
  used for normalization and subset comparison. It MUST be present even if only
...
### 6.7 Registry artifacts are defined in PSP-4
...
### 10.5 Structure and Informative Projection
...
- **exp**: Int (NumericDate seconds) - expiry; `exp - iat` SHOULD be short; MAY
  be bounded by the Program via `ttlOk`.
...
explicit in the Program structure. See
[Section 14, _Security Considerations_](#14-security-considerations) for
additional discussion of the risks mitigated by syntactic attenuation.

### 11.3 Chain Structure and Custody
...
### 11.4 Pinned Semantics Compatibility Across Hops
...
### 11.6 Closed-World Enforcement and Availability
