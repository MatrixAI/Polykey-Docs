...
- **Presentation (Π):** An ephemeral proof-of-possession (PoP) token created by
  S that references a Grant by its canonical digest (grantRef, per PSP-3). It is
  bound to a live channel and runtime context and is NOT stored on sigchains.
...
- **Pin:** A self-describing, content-addressed identifier (e.g., multihash/CID)
  ...
  (e.g., builtinsId, channelLatticeId, schemesSnapshotId). CEPs MUST recognize
  required pins and, across delegation chains, matching pins are REQUIRED where
  they affect evaluation.
- **CEP Placement Variants (CEP(R), CEP(P), CEP(S)):** A CEP may operate in
  different trust domains depending on where verification occurs.
  - **CEP(R)** denotes a _Resource-side_ CEP embedded in or colocated with the
    target Resource.
  - **CEP(P)** denotes a _Principal-side_ CEP operated within the issuer's
    (Principal's) domain, often used when bridging to a non-PK-native resource.
  - **CEP(S)** denotes a _Subject-side_ CEP operated within the Subject's
    domain; it may manage derived credentials or session keys. These placement
    variants are defined in the CEP/BA specification and are referenced
    throughout PSP-1.
- **Bridge Adapter (BA):** A specialized CEP that bridges to a non-PK-native
  Source of Authority (SoA) using a long-lived upstream lease. The two
  recognized subtypes are **PS-BA** (Principal-Side Bridge Adapter), where the
  Principal holds the upstream lease, and **SS-BA** (Subject-Side Bridge
  Adapter), where the Subject holds the lease. The BA enforces _exposure modes_
  when interacting with legacy systems:
  - **mediate** - the adapter performs the downstream call on behalf of the
    Subject; the secret never leaves the adapter.
  - **derive** - the adapter mints a short-lived, scoped token for the Subject
    to use directly.
  - **reveal** - the adapter returns the raw secret to the Subject under strict,
    TAP-governed constraints. Exposure modes and adapter placements are defined
    in the CEP/BA specification.
...
- Each builtin op_id, its type signature, and its tightening rule are defined in
  the Builtins registry (PSP-4). The exact set in use is pinned by builtinsId in
  the Grant.
- Channel comparisons (e.g., channel_geq) consult the pinned channelLatticeId,
...
- Structural mapping (CPL/0 -> Biscuit):
  ...
  - Query (AND of Literals) ? a Biscuit query whose predicates match CPL/0
    literals 1:1.
  - Literal (Builtin(op, args...)) ? a ground predicate recognized by the
...
- Conversion constraints (Biscuit -> CPL/0 normalizer):
  ...
  2. All predicate identifiers MUST be recognized builtin op_ids in the pinned
     builtinsId; argument arity/types MUST match; strings are NFC; Bytes are
     exact octets.
...
- channelLatticeId is the content-addressed ID of the specific lattice used to
...
- schemesSnapshotId: Grants pin a content-addressed manifest that maps
  each scheme name referenced by declarations to the exact comparator artifact
  used for normalization and subset comparison. CEPs select comparator
  semantics by scheme name within this pinned snapshot. Unknown or unavailable
  comparators at verification MUST cause deny.
...
- builtinsId: ID of the Builtins set used by the Program.
...
- The Program uses channel_geq but the pinned channelLatticeId does not match
...
- A Grant carrying programBytes and programId MUST be rejected if recomputation
  does not match.
...
1. Payload contents
   - programBytes: The deterministic encoding of PCF(Program) as specified by
     PSP-3.
   - programId: The multihash of programBytes (PSP-3). CEPs MUST recompute and
     match.
   ...
   - pins: Registry pins that fix semantics across CEPs and time:
     - langVersion: The CPL/0 language version string (e.g., "cpl/0@1" - string
...
3. Program identity and integrity
   - Implementations MUST construct PCF(Program) prior to deriving programId.
   - At verification, the CEP MUST recompute programId from programBytes. Any
...
4. Registry pinning and compatibility
   ...
   - If the Program contains channel_geq, channelLatticeId MUST be present and
   ...
   - In a delegation chain, a child Grant's langVersion and builtinsId MUST
     ...
     child, channelLatticeId (when present) MUST be equal across the chain. Any
     mismatch MUST cause deny.
...
6. Fail-closed behavior
   - If the Program references a builtin not present in builtinsId, verification
     MUST deny.
   - If the Program uses channel_geq but channelLatticeId is missing or unknown,
...
- Declarations subset:
  ...
  - Registry pins:
  - langVersion_child == langVersion_parent.
  - builtinsId_child == builtinsId_parent.
  - If channel_geq is present in either parent or child, channelLatticeId_child
    == channelLatticeId_parent.
...
```json
{
  ...
  "pins": {
    ...
    "schemesSnapshotId": "cid:schemes@YYYYMMDD" // REQUIRED
  }
}
```
...
- Delegation material (TAP-governed):
  - Presentations in PSP-1 core MUST carry only grantRef (leaf digest). They MAY
    include grantRefs: an ordered list of parent digests (leaf->root) as
...
- Grant reference and custody
  ...
  - If delegation is present, the Presentation MUST carry (or, per TAP, make
    ...
    - pins compatibility (langVersion, builtinsId, and channelLatticeId when
      channel_geq is used),
...
- If channel_geq appears in either parent or child, channelLatticeId_child MUST
  equal channelLatticeId_parent.
...
- Pins incompatibility:
  - langVersion mismatch; builtinsId mismatch; channelLatticeId mismatch when
    channel_geq is used by parent or child.
- Unknown or unavailable semantics:
  - Program references a builtin not present in builtinsId; Program uses
    channel_geq but channelLatticeId is missing or unknown; any resource scheme
...
- Presentation P: presenter, grantRef, iat, exp, jti, channelBinding, ctx, and
...
1. Parse Presentation
   - Validate required conceptual fields: presenter, grantRef, iat, exp, jti,
     channelBinding, ctx.
   - Capture a single logical now at the start of enforcement. Enforce the
     Presentation lifetime window using this now: `iat <= now < exp` (subject to
     TAP clock discipline); else deny.
...
4. Verify delegation chain (if applicable)
   ...
   - For each hop child -> parent:
     ...
     - Pinned semantics compatibility: langVersion matches; builtinsId matches;
       channelLatticeId matches whenever channel_geq is used by either hop;
       schemesSnapshotId matches; else deny.
...
6. Load Program and Declarations from the leaf Grant
   ...
   - Confirm pinned semantics are usable: builtinsId known; if the Program uses
     channel_geq, channelLatticeId present and known; schemesSnapshotId present
     and known; else deny.
...
8. Decision and receipts (placement-agnostic)
   - On success:
     ...
     - The enforcing CEP MUST emit an Access PoAR per PSP-2 on its own sigchain
       ...
       include programId, declaration CIDs, pins (including schemesSnapshotId),
       a minimal evaluation trace (which check/query passed), and
       revocation/freshness decision context as applicable. For audit
       acceleration, the PoAR SHOULD also record comparator fingerprints (per
       scheme) actually used during evaluation.
   - On failure:
...
- Canonicalization and pinning
  ...
  - Pins affecting semantics (builtinsId, and channelLatticeId when channel_geq
    is used) MUST match across delegation hops; mismatches MUST deny.
...
- Input bounds
  - Programs and Declarations MUST remain finite. CEPs SHOULD bound sizes of
    programBytes, declaration tables, and ctx to mitigate memory/CPU pressure.
...
- Field names in the JSON projections below use **camelCase** for readability.
  These labels are **illustrative only**; the normative field names and
...
- channelLatticeId: "cid:channel-lattice@v1" (required because channel_geq is
  used)
...
exact set via **builtinsId**. This appendix is normative for this edition;
implementers MUST ensure builtinsId references a set that matches these
definitions.
...
- Unknown builtin op_id referenced by a Program under the pinned builtinsId MUST
  cause deny.
...
- pcf_mismatch
- program_parse_error
- pin_mismatch (langVersion / builtinsId / channelLatticeId)
- unknown_builtin
- unknown_lattice
- unknown_comparator
...
Receipts SHOULD include enough context (programId, declaration CIDs, pins,
